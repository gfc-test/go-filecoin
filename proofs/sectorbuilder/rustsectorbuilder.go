...
func (sb *RustSectorBuilder) GetMaxUserBytesPerStagedSector() (uint64, error) {
	defer elapsed("GetMaxUserBytesPerStagedSector")()

	size, err := cFFISectorSizeFrom(sb.SectorClass.SectorSize())
	if err != nil {
		return 0, errors.Wrap(err, "CSectorStoreType failed")
	}

	return uint64(C.get_max_user_bytes_per_staged_sector(size)), nil
}
