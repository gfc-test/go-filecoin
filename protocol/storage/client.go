...
func (smc *Client) getSectorSize(ctx context.Context) (uint64, error) {
	...
	values, err := smc.api.MessageQuery(
		ctx,
		address.Address{},
	...
	if err != nil {
		return proofs.SectorSize(proofs.TestMode), errors.Wrap(err, "'getProofsMode' query message failed")
	}

	if err := cbor.DecodeInto(values[0], &proofsMode); err != nil {
		return proofs.SectorSize(proofs.TestMode), errors.Wrap(err, "could not convert query message result to Mode")
	}

	return proofs.SectorSize(proofsMode), nil
}
