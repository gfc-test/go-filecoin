...
func TestPieceRejectionInProposeStorageDeal(t *testing.T) {
	...
	dataCid := clientDaemon.RunWithStdin(bytes.NewReader(byteSlice), "client", "import").ReadStdoutTrimNewlines()

	proposeDealOutput := clientDaemon.Run("client", "propose-storage-deal", fixtures.TestMiners[0], dataCid, "0", "5").ReadStdoutTrimNewlines()

	assert.NotContains(proposeDealOutput, "accepted")
}
